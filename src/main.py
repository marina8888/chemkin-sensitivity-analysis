...
def main():
    ...
    # Create a new graph:
    graph = create_graphs.Graph('20% Heat Ratio, 1.15 Equivalence Ratio')

    # Find list of equations (eq) within the filter boundaries for species sensitivity NO, by using 'do_not_plot':
    eq = graph.plot_sensitivity(df2, 'NO', X = 2.0, filter_below=-0.01, filter_above=0.01, sorting=True, do_not_plot=True)
    ...
    eq = graph.plot_sensitivity('data/okafor_laminar/20%_1.15.csv', list_of_eq=eq, offset=1, X=0, colour ='r', multiplier=1.5, legend ='Laminar burning velocity x 1.5', sorting = True)
    ...
    graph.show_and_save('./output/graph', '1.15_20.png')
