...
def main():
    ...
    df = prepare_sheet.add_distance('data/okafor_stag_sens/okafor_stag_1.15_0.4.csv', 0.02)
    df_lam = prepare_sheet.add_distance('data/okafor_lam_sens/okafor_lam_1.15_0.4.csv', 0.02)
    ...
    NO_equations = graph.plot_bar_species(df, 'NO', filter_below=-0.03, filter_above= 0.03, X = 0.02, offset = 0.3, colour = 'g', sorting = True)

    #Add laminar burning velocity plot spreadsheet, using equations filtered in the previous plot:
    graph.plot_bar_species(df, 'NH3', list_of_eq = NO_equations, X = 0.02, offset = 0.15, multiplier=0.5)
    graph.plot_bar_lam_burning_v(df_lam, list_of_eq = NO_equations, X = 0, sorting=True)

    #Save graphs to ./output/graphs folder (which user may need to create) under the name test.png:
    graph.show_and_save('./output/graphs', '1point15_40%')
