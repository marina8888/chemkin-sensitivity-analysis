...
def main():
    ...
    NO_equations = graph.plot_bar_species(df, 'NO', X = 0.02, colour = 'g', filter_above=0.01, filter_below=-0.01, offset = 0.15, sorting = True)

    #Add laminar burning velocity plot spreadsheet, using equations filtered in the previous plot:
    # NH3_equations = graph.plot_bar_species(df, 'NH3', list_of_eq = NO_equations, X = 0.02)
    # lam_eq = graph.plot_bar_lam_burning_v(df_lam, list_of_eq = NO_equations, X = 0,  offset = -0.15)
